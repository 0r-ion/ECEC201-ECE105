...
def plot_eff(s_num, h_num, b_num, k_num, r_set, l_set, eta_ave, filename):
    ...
    # print(f"l_set:{l_set}")
    # print("----------")
    # print(f"eta_ave:{eta_ave}")
    fig = plt.figure()
    ax = fig.add_subplot(111)
    # 2. plot the average efficiency under the three strategies vs. r_set
    ax.scatter(r_set,eta_ave[0])
    ax.scatter(r_set,eta_ave[1])
    ax.scatter(r_set,eta_ave[2])
    ax.plot(r_set,eta_ave[0],'-',label='best')
    ax.plot(r_set,eta_ave[1],'-',label='est')
    ax.plot(r_set,eta_ave[2],'-',label='rand')
    # 3. also use scatter plot to show the points on the plot
    # 4. add labels and title and legend
    ax.set_title(f"Winning efficiencies(s_num={s_num}, h_num={h_num}, b_num={b_num}, k_num={k_num})")
    ax.set_ylabel("Winning efficiencies")
    ax.set_xlabel("Number of horse races (r_num)")
    ax.legend()
