...
def fwia(a, w):
    ...
    for i in range(a.shape[0]):
        ...
        flippedW = np.flip(w)
        if bwil(cur, w) == True:
            # print(f"word: {''.join(w)} in {''.join(cur)}")
            si, ei = pwil(cur, w)
            rc = 'r'
            rci = i
        elif bwil(cur, flippedW) == True:
            # print(f"word {''.join(flippedW)} (reversed) in {''.join(cur)}")
            si, ei = pwil(cur, flippedW)
            rc = 'r'
            rci = i
    ...
    # print(f"    rc:{rc}		rci:{rci}	si:{si}		ei:{ei}")
    return rc, rci, si, ei
	#no rev col
	#
...
if __name__ == "__main__":
    ...
    print("\nTarget word list:")
    printArray(np.array(wl, dtype=object))
