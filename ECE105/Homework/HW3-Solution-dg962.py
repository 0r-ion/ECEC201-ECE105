...
ASSIGNMENT: COMPLETE function get_letter_disbn below
"""
...
This code creates a "sentence" using Mandelbrot's random construction.  It
leverages the stick-breaking distribution for letter frequencies as found in:

Richard Perline, "The random division of the unit interval and the approximate
-1 exponent in the monkey-at-the-typewriter model of Zipf's law", submitted to
...
def get_letter_disbn(alphabet):
    ...
    n = len(alphabet)-1
    uniforms = sorted([np.random.uniform() for i in range(n)])
    # generate n-1 independent uniform RVs on [0,1]
    # 2. sort them from smallest to largest
    placeholder = [0] + uniforms +[1]
    sorts = sorted(np.diff(placeholder))
    # 3. prepend with 0, append with 1, and take the K differences
