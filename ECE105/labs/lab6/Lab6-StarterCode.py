...
ASSIGNMENT: COMPLETE asssign(i) function below
"""
...
def blocks(i):
    b_l = [i[0:3, 0:3], i[0:3, 3:6], i[0:3, 6:9], i[3:6, 0:3],
           i[3:6, 3:6], i[3:6, 6:9], i[6:9, 0:3], i[6:9, 3:6], i[6:9, 6:9]]
...
def valid(i):
    ...
    if not all([valid_set(set(r)) for r in i]):
        return False
    # return False if there are any invalid columns
    if not all([valid_set(set(c)) for c in i.T]):
        return False
    # return False if there are any invalid blocks
    if not all([valid_set(set(b)) for b in blocks(i)]):
        return False
